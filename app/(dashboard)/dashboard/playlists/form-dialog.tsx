...
import { playlistFormDefaultValues, playlistSchema, type PlaylistFormValues } from "./schema";
...
export function PlaylistFormDialog({
  ...
  useEffect(() => {
    ...
    if (editing) {
      ...
      form.reset(playlistFormDefaultValues);
    }
  }, [open, editing, form]);
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <div className="flex items-center justify-between rounded-md border border-input p-3">
                    <div className="space-y-0.5">
                      <FormLabel className="m-0">系统歌单</FormLabel>
                      <p className="text-xs text-muted-foreground">
                        标记为官方运营歌单，用于发现页推荐
                      </p>
                    </div>
...
// ==================== 歌单内歌曲管理弹窗 ====================
export interface PlaylistSongsDialogProps {
  open: boolean;
  playlist: Playlist;
  onOpenChange: (open: boolean) => void;
}

export function PlaylistSongsDialog({
  open,
  playlist,
  onOpenChange,
